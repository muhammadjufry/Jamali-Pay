...
function Cookie_Card({}: Props) {
  ...
  useEffect(() => {
    if (typeof window !== null) {
      ...
      } else if (isCookieAllowed === "declined") {
        setIsAllowedCookie("declined");
      } else {
        setIsAllowedCookie("false");
      }
    }
  }, [isAllowedCookie]);

  const allowCookie = () => {
    localStorage.setItem("is_cookie_allowed", "true");
    setIsAllowedCookie("true");
  };

  const declineCookie = () => {
    localStorage.setItem("is_cookie_allowed", "declined");
    setIsAllowedCookie("declined");
  };
  return (
    <>
      {isAllowedCookie === "false" ? (
